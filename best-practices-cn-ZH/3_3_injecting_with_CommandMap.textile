h3(#mappingwithcommandmap). CommandMap类的依赖注入

CommandMap类实现ICommandMap接口, 提供一个用来映射commands到触发它们的框架事件的方法.

<pre>mapEvent(eventType:String, commandClass:Class, eventClass:Class = null, oneshot:Boolean = false)</pre>

你要提供给commandMap一个可以执行的类, 执行它的事件类型, 可选的这个事件的强类型, 以及这个command是否只被执行一次并且随即取消映射的布尔值开关.

这个可选的强类型事件类用来对Flash平台的"magic string"事件类型系统做额外的保护.以避免不同事件类采用相同事件类型字符串的冲突.
